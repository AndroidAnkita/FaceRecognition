...
import org.tensorflow.lite.examples.detection.database.UserEntity;
...
// DATABASE SAVE
        ...
        byte[] faceData = SimilarityUtils.floatArrayToByteArray(emb);

        UserEntity user = new UserEntity(name, faceData);
    ...
    Executors.newSingleThreadExecutor().execute(() -> {
      ...
      runOnUiThread(() -> {
        ...
        for (UserEntity user : users) {
          ...
          float[][] emb =
                  SimilarityUtils.byteArrayToFloatArray(user.faceData);

          SimilarityClassifier.Recognition rec =
                  new SimilarityClassifier.Recognition(
                          "0", user.name, 0f, null);
